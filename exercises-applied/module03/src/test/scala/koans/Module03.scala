...
class Module03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  // 2. an auxiliary constructor that takes just a real double and creates a
  ...
  // UNCOMMENT BELOW
  class ComplexNum private (val real: Double = 0.0, val imaginary: Double = 0.0){
    override def toString: String = {
      s"$real ${if (imaginary > 0.0) "+" else "-"} ${abs(imaginary)}i"
    }
  ...
  object ComplexNum {
    ...
    implicit def apply(real: Double): ComplexNum = {
      new ComplexNum(real, 0.0)
    }
  }
